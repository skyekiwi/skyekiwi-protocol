...
class IPFS {
    ...
    async init() {
        const logger = index_1.Util.getLogger('ipfs.init');
        try {
    ...
    async stopIfRunning() {
        const logger = index_1.Util.getLogger('ipfs.stopIfRunning');
    ...
    async add(str) {
        const logger = index_1.Util.getLogger('ipfs.add');
    ...
    async addAndPinInfura(content) {
        const logger = index_1.Util.getLogger('ipfs.addAndPinInfura');
    ...
    async addAndPinSkyeKiwi(content) {
        const logger = index_1.Util.getLogger('ipfs.addAndPinSkyeKiwi');
    ...
    async remoteGatewayAddAndPin(content) {
        const logger = index_1.Util.getLogger('ipfs.remoteGatewayAddAndPin');
        try {
    ...
    async fetchFileFromRemote(cid) {
        const logger = index_1.Util.getLogger('ipfs.fetchFileFromRemote');
        try {
            const requests = [
