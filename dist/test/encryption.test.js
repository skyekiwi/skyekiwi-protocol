...
describe('Encryption', () => {
    ...
    const message_u8a = util_1.stringToU8a(message);
    ...
    it('Symmetric: Encryption & Decryption Works', () => {
        ...
        const decrypted_string = util_1.u8aToString(decrypted);
    ...
    it('Symmetric: Decryption Fails w/Wrong Key', () => {
        ...
        const encrypted = SkyeKiwi.SymmetricEncryption.encrypt(key, message_u8a);
        chai_1.expect(() => SkyeKiwi.SymmetricEncryption.decrypt(wrong_key, encrypted)).to.throw("decryption failed - SecretBox.decrypt");
    });
    it('Asymmetric: Decryption Fails w/Wrong Key', () => {
        ...
        chai_1.expect(() => SkyeKiwi.AsymmetricEncryption.decrypt(receiver_privateKey, encrypted, receiver_publicKey)).to.throw('decryption failed - Box.decrypt');
        // wrong receiver's private key
        const wrong_private_key = tweetnacl_1.randomBytes(32);
        chai_1.expect(() => SkyeKiwi.AsymmetricEncryption.decrypt(wrong_private_key, encrypted, sender_publicKey)).to.throw('decryption failed - Box.decrypt');
    });
