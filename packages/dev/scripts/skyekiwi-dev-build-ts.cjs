...
function buildWebpack () {
  ...
  execSync(`yarn skyekiwi-exec-webpack --config ${config} --mode production`);
}
...
async function buildJs (dir) {
  if (!fs.existsSync(path.join(process.cwd(), '.skip-build'))) {
    ...
    mkdirp.sync('build');
    fs.writeFileSync(path.join(process.cwd(), 'src/packageInfo.ts'), `// Copyright 2021 ${name} authors & contributors
// SPDX-License-Identifier: Apache-2.0
...
async function main () {
  execSync('yarn skyekiwi-dev-clean-build');
  ...
  execSync('yarn skyekiwi-exec-tsc --emitDeclarationOnly --outdir ../build');
  ...
  if (['js', 'mjs', 'cjs'].some((e) => fs.existsSync(path.join(process.cwd(), `rollup.config.${e}`)))) {
    execSync('yarn skyekiwi-exec-rollup --config');
  }
}
